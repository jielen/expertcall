...
//	private static final String UPDATE_CALL_SERVER_STATE = "update EM_CALL_SERVER_LIST set ISCALL = ISCALL+1 where OBJID=?";
	...
	private static final String UPDATE_EM_CALL_SERVER_LIST_TO_FAIL = "update EM_CALL_SERVER_LIST set ISCALL = -1 where OBJID=?";
 ...
 */
	 ...
	 * 呼叫专家的次数，默认是给专家打3次电话
	 */
	public static int CALL_NUM = new Integer(3);
	...
	public void scan() throws Exception {
		...
		if (!f.exists()) {			
			...
			if (!flag) {
				...
				throw new Exception("呼叫退出，生成语音文件失败：" + playFilePath);
			}
		}		
		
		logger.debug("第" +  callNum + "次呼叫电话:"+ emMobile);		
	 ...
	 * @throws SQLException 
	 */
	private boolean isSelecting(String emBillCode) throws SQLException {
	...
	private String getBillStatus(String emBillCode) throws SQLException{
		String status=null;
		String sql="select b.em_bill_status from zc_em_expert_pro_bill b where b.em_bill_code=?";
		Object[] params=new Object[]{emBillCode};
		Map<String, String> rt=DAOFactory.queryToColumnMap(sql,params);
		if(rt==null){
			return status;
		}
	...
	private boolean pauseSelecting(String emBillCode) throws SQLException {
		String status=getBillStatus(emBillCode);
	...
	public String getKeyStr(int ch) throws NativeException,
			IllegalAccessException {
		Pointer pointer = SSMFactory.creatPointer(40);
		String key = "";
	...
	public void call(String emMobile, String playFilePath, String emExpertCode,
			String objId, String emBillCode, int isCall,String expertType) throws Exception {
		String key = "";
		int record = 0;
		int rlt = -2;
		int ch = -1;
		try {
			...
			if (ch == -1) {
				logger.info("获取空闲通道失败。" + getLastErrMsg());
			...
			if (rlt == -1) {
				logger.info("设置参数失败。" + getLastErrMsg());
				updateCallingStatus(CallServer.YUYIN_TONGDAO_YICHANG, key,emExpertCode, objId, emBillCode, isCall, record,expertType);
				return;
			}
			...
			if (rlt == -1) {
				logger.info("摘机失败。" + getLastErrMsg());
			...
			if (rlt == -1) {
				logger.info("呼叫失败。" + getLastErrMsg());
				SSMFactory.ssmHangup(ch);
			...
			if (chkAuto == -1 || timmer == 30000 || chkAuto == 12) {
				logger.info("专家(编号:"+emExpertCode+")电话号码为空号，请核对专家库中的专家电话号码！");
				SSMFactory.ssmHangup(ch);
			...
			if (rlt == -1) {
				logger.info("清空按键缓冲区失败。" + getLastErrMsg());
				SSMFactory.ssmHangup(ch);
			...
			if (rlt == -1) {
				logger.info("挂机失败。" + getLastErrMsg());
				//return;
			}
	 ...
	 * @throws Exception
	 */
	void updateCallingStatus(String callStatus, String key,
			String emExpertCode, String objId, String emBillCode,
			int isCall, int record,String expertType) throws Exception {
		...
		if(callStatus.equals(CallServer.CALL_STATUS_BEGIN)){//开始呼叫		
			...
			String sql=CallServer.UPDATE_EM_CALL_SERVER_LIST_TO_FAIL;
			Object[] params = new Object[] {Integer.parseInt(objId)};
			sqlList.add(sql);
			paramList.add(params);
	...
	private void excuteSql(List<String> sqlList, List<Object[]> paramList)
			throws Exception {
		// TODO Auto-generated method stub
		...
		if (paramList == null || sqlList.size() != paramList.size()) {
			throw new Exception("更新拨打状态的脚本不匹配");
		}
		...
		try {
			...
			for (int i = 0; i < sqlList.size(); i++) {
				...
				DAOFactory.executeUpdate(conn, sql, params);
			}
			conn.commit();
		} catch (Exception e) {
			if (conn != null) {
